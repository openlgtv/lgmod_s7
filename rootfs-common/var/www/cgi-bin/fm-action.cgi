...
<?
# mmm4m5m: adapted for Saturn6/Saturn7 - notes:
# in: /var/www/cgi-bin/
# replace: /bin/haserl -> /usr/bin/haserl
# replace: /var/log/ -> /tmp/fm/
?>
...
then
	...
	echo "</table><br/>"
	echo "${action}#${spth}#$dpth" >> /tmp/fm/${action}.log
	echo "<br/>"
	SIFS="$IFS" IFS=$'\n'
	...
	then
	    if [ "$action" = "copy" ]
	    then
		cp -r "$spth" "$dpth/" > /tmp/fm/${action}.log 2>&1 &
		pid="$!"
	    else
		mv "$spth" "$dpth/" > /tmp/fm/${action}.log 2>&1 &
		pid="$!"
	    fi
	    date +"%s" > /tmp/fm/${action}.date.${pid}.log
	fi
	time_start="`cat /tmp/fm/${action}.date.${pid}.log`"
	...
	do
		...
		then
		    ...
		    if [ -f "/tmp/fm/${action}.log" ]
		    then
			echo "<font color='red' size='+2'>"
			cat /tmp/fm/${action}.log
			echo "</font>"
		    fi
		    error=1
		    break
