...
<body><hr><p class="largefont">LGMOD CONFIGURATION / NETWORK</p>
...
<? ip=`awk '{print $1}' /mnt/lg/user/lgmod/network`
 ...
 echo "Subnet Mask:<input name="mask" type="text" id="mask" $flag value="$mask">"
 echo "Gateway: <input name="gw" type="text" id="gw" $flag value="$gw"><br><br>"
 echo "<b>DNS Server:</b><input name="dns" type="text" id="dns" value="$dns">"
?></div></div>
...
<b>Launch telnetd service at startup:</b>
...
<? tz=`awk '{print $1}' /mnt/lg/user/lgmod/TZ`
 ...
 echo "<input name="ntp" type="radio" value="0" onclick="nip.disabled=true\;tz.disabled=true"$no><b>Disable</b><br>"
 echo "Server IP/Name:<input name="nip" type="text" id="nip" $flag value="$nip">"
 echo "Timezone (e.g GMT-3):<input name="tz" type="text" id="tz" $flag value="$tz"><br>"
?>
