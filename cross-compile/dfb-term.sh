...
d=sources; mkdir -p $d; cd $d; SRC_dir=DFBTerm-0.8.15; SRC_DIR="$(pwd)/$SRC_dir"
...
if [ ! -d "$dir" ]; then
	...
	[ -f "$tar" ] || { wget "http://directfb.org/downloads/Programs/$tar" || exit 3; }
	tar -xzf "$tar"
	exit
fi
...
export DIRECTFB_LIBS="-L${SRC_DIR%/*}/DirectFB-LG-usr_local_lib -ldirectfb -lfusion -ldirect -lpthread -lz"
export LITE_CFLAGS="-I${SRC_DIR%/*}/LiTE-0.8.10 -DLITEFONTDIR="'\"${datarootdir}/fonts/truetype\"'
export LITE_LIBS="-L${SRC_DIR%/*}/LiTE-0.8.10/lite/.libs -llite"
